...
export default function PropuestasEnviadasScreen() {
  ...
  const loadPropuestas = async () => {
    try {
      ...
      Alert.alert('Error', 'No se pudieron cargar las propuestas enviadas');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleCancelar = async (id, articuloOfrecido) => {
    Alert.alert(
      'Cancelar propuesta',
      `¿Cancelar la propuesta de "${articuloOfrecido}"?`,
      [
        { text: 'No', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Propuesta cancelada', 'Tu propuesta ha sido cancelada');
              loadPropuestas();
            } catch (error) {
              Alert.alert('Error', 'No se pudo cancelar la propuesta');
            }
          },
        },
      ]
  ...
  const renderPropuesta = ({ item }) => (
    <View style={styles.propuestaCard}>
      <View style={styles.propuestaHeader}>
        <Text style={styles.propuestaTitulo}>{item.articulo_solicitado?.titulo}</Text>
        {renderEstadoBadge(item.estado)}
      </View>

      <View style={styles.propuestaBody}>
        <View style={styles.intercambioInfo}>
          <Text style={styles.label}>Ofreces:</Text>
        ...
        <View style={styles.intercambioInfo}>
          <Text style={styles.label}>Por el artículo de:</Text>
          <Text style={styles.valueBold}>
            {item.usuario_solicitado?.nombre} {item.usuario_solicitado?.apellido}
          </Text>
        </View>

        <View style={styles.intercambioInfo}>
          <Text style={styles.label}>Artículo solicitado:</Text>
      ...
      {item.estado === 'pendiente' && (
        <TouchableOpacity
          ...
          onPress={() => handleCancelar(item.id, item.articulo_ofrecido?.titulo)}
        >
          <Ionicons name="close-circle" size={18} color="#FFF" />
          <Text style={styles.buttonText}>Cancelar propuesta</Text>
        </TouchableOpacity>
      )}
      ...
      {item.estado === 'aceptada' && (
        <View style={styles.successMessage}>
          ...
          <Text style={styles.successText}>¡Tu propuesta fue aceptada!</Text>
        </View>
      )}
      ...
      {item.estado === 'rechazada' && (
        <View style={styles.rejectedMessage}>
          ...
          <Text style={styles.rejectedText}>Propuesta rechazada</Text>
        </View>
      )}
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Propuestas Enviadas</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>No has enviado propuestas aún</Text>
          </View>
        }
      />
    </View>
